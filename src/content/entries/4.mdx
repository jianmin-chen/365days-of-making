...
Okay, so I didn't end up using [Satori](https://github.com/vercel/satori). It's a great concept and super easy to use, but didn't end up working fully well because I couldn't get it working with modern CSS like `repeating-linear-gradient`. I ended up using [`node-html-to-image`](https://www.npmjs.com/package/node-html-to-image), which works pretty well since we already run the Slack bot on a Heroku server.

## More Slack bots?

I am going to write a Clue bot! I'm planning to do a #ship post sometime on building Slack bots and explaining how the pizza bot is build with a Jam, but now that I know how to build Slack bots I want to try my hand at building a Clue bot. Will update in the following days.
