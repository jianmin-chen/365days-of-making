...
Move `setup.ts` into `/pages/api/auth`. We're going to set up an admin account with your personal email (or as many as you'd like, so make sure to use the appropriate email. Or you could create multiple logins. Whatever you'd like.)
...
Create a new request with the Add button and then set it to be `POST` instead of the default `GET`. Go over to the Body tab and select `form-data`, and add these two key-value pairs:
...
If everything works out, you now have an account! Delete the `setup.ts` file and head to `/signin`, and you should be able to sign in, after which you will get redirected to the dashboard.
...
Now that you've got everything set up, let's write our first entry. This can be about anything! For me, I introduced myself, what this was about, why I was doing it, and what I was going to be doing. [You can check it out here.](/entry/1)
...
One last thing before we deploy - did you know you can search for things with `Cmd+k` or `Ctrl+k`? I mean, it works on this site, but not on yours, because you haven't set it up. We use [Stork](https://stork-search.net/) for static search, which makes it insanely fast because all search indexes have already been built. To use it, first [install Rust](https://www.rust-lang.org/tools/install), and then run `npm run search`. **Keep in mind that this only works on Linux and macOS computers.**
